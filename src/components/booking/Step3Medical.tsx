...
const flow: Record<ConditionId, Question[]> = {
  ...
  fracture: [
    { key: "place", label: "مكان الإصابة", options: ["الظهر", "الرقبة", "الكتف", "الذراع", "الكوع", "اليد", "الورك", "الفخذ", "الركبة", "الكاحل", "القدم", "أخرى"] },
  ...
  mobility: [
    { key: "cause", label: "سبب المشكلة؟", options: ["جلطة دماغية", "مرض الرعاش", "تنويم طويل بالمستشفى", "أمراض العضلات", "مرض التصلب المتعدد", "أخرى"] },
    { key: "issueType", label: "نوع المشكلة", options: ["ضعف عضلي", "ضعف أسفل الجسم", "ضعف نصف الجسم", "مشكلة في التوازن", "مشكلة في الحركة", "صعوبة في المشي"] },
...
function expandQuestions(base: Question[], medical: Record<string, string>, problem: ConditionId): Question[] {
  ...
  for (const q of base) {
    ...
    if (q.key === "surgery" && problem === "fracture" && medical.surgery === "نعم") {
      result.push({
        key: "surgeryWhen",
